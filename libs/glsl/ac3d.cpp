...
namespace glsl { 
  ...
  namespace {
    ...
    class acstring {
    public:
      explicit acstring(std::istream& is) {
        char c;
        is >> c >> c;    // trim leading '"'
        while (c != '"') {
          str_ += c;
    ...
    { 
      objects_.back().texture = acstring(is_).str();
    }


    void acParser::read_name() 
    { 
      objects_.back().name = acstring(is_).str();
    }
    ...
    {  
      throw std::runtime_error((std::string("Parsing AC3D file '") + acFileName_ + "' failed").c_str()); 
    }
    ...
    {
      ac3d_model model;
      pos_vect v; TranslateObj(objectID, glm::mat4(1), v);// glm::rotate(glm::mat4(1), 0.0f, glm::vec3(0, 1, 0)), v);
